...
function routeFromType(type: string) {
  const map: Record<string, string> = {
    TRUYEN_CHU: "truyen-chu", MANGA: "manga", MANHWA: "manhwa",
    MANHUA: "manhua", BL_COMIC: "truyen-tranh-bl", PHIM_ANIME: "phim-anime",
    DOUJINSHI: "doujinshi", FANFIC: "fanfic",
  };
  return map[type] ?? "";
}
...
export async function ContentBrowser({
  ...
  const [totalCount, filteredCount, latestUpdated, items] = await Promise.all([
    ...
    prisma.content.findMany({
      ...
      take: PAGE_SIZE,
      select: {
        id: true, title: true, type: true, imageUrl: true,
        purityTags: true, status: true, genres: true, updatedAt: true,
        author: true, fanficTitle: true, doujinshiTitle: true, whereToAccess: true,
      },
    }),
  ]);
  ...
  return (
    <div className="space-y-8">
      ...
      <form className="flex flex-wrap gap-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
        <input
          name="q" defaultValue={q} placeholder="Tìm kiếm..."
          className="min-w-[200px] flex-1 rounded-lg border border-zinc-200 px-4 py-2 text-sm outline-none focus:border-zinc-400"
        />
        <select
          name="status" defaultValue={status}
          className="rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm outline-none focus:border-zinc-400"
        >
          <option value="">Tình trạng</option>
          {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        <input
          name="purity" defaultValue={purity.join(",")} placeholder="Độ khiết (SK, Toàn khiết...)"
          className="rounded-lg border border-zinc-200 px-4 py-2 text-sm outline-none focus:border-zinc-400"
        />
        <input
          name="genre" defaultValue={genre.join(",")} placeholder="Thể loại (Romance,Fantasy...)"
          className="rounded-lg border border-zinc-200 px-4 py-2 text-sm outline-none focus:border-zinc-400"
        />
        <button
          type="submit"
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {items.map(item => (
            ...
            >
              <div className="mb-3 aspect-[3/4] overflow-hidden rounded-lg bg-zinc-100">
                {item.imageUrl ? (
              ...
              <div className="space-y-2.5">
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${badgeClass(item.type)}`}>
                    {CONTENT_TYPE_LABELS[item.type] ?? item.type}
                  </span>
                  {item.status && (
                    <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${statusClass(item.status)}`}>
                      {item.status}
                    </span>
                  )}
                </div>
                <h3 className="text-sm font-semibold leading-snug text-zinc-900 group-hover:text-zinc-600 line-clamp-2">
                  {item.title}
                </h3>
                ...
                {(item.genres ?? []).length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {item.genres.map(g => (
                      <span key={g} className="inline-flex items-center rounded-md border border-zinc-200 bg-white px-2 py-0.5 text-xs text-zinc-600">
                        {g}
                      </span>
                    ))}
                  </div>
                ...
                {(item.purityTags ?? []).length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {item.purityTags.slice(0, 3).map(tag => (
                      <span key={tag} className="rounded-md px-1.5 py-0.5 text-xs" style={purityTagStyle(tag)}>{tag}</span>
                    ))}
                  </div>
                )}
                ...
                <div className="text-xs text-zinc-400">
                  ...
                  {item.fanficTitle && <span className="ml-2">Fanfic: {item.fanficTitle}</span>}
                  {item.doujinshiTitle && <span className="ml-2">Doujinshi: {item.doujinshiTitle}</span>}
                </div>
              </div>
            </Link>
          ))}
...
function PaginationLink({ href, disabled, children }: { href: string; disabled?: boolean; children: React.ReactNode }) {
  const baseClass = "rounded-lg border px-4 py-2 text-sm font-medium transition";
  if (disabled) {
    return <span className={`${baseClass} cursor-not-allowed border-zinc-100 bg-zinc-50 text-zinc-300`}>{children}</span>;
  }
  return (
